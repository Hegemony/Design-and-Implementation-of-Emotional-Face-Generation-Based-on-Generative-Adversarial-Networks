...
class BaseModel:
    ...
    def setup(self):
        ...
        if self.is_train:
            ...
            self.criterionGAN = model_utils.GANLoss(gan_type=self.opt.gan_type).to(self.device)  # gan_type默认是wgan-gp
    ...
    def get_latest_visuals(self, visuals_name): # 返回一个visual_ret字典，里面key为要展示的图片,value为值
    ...
    def get_latest_losses(self, losses_name):  # 返回一个errors_ret字典，里面key为loss,value为值
    ...
    def get_latest_losses1(self, losses_name):
        errors_ret = OrderedDict()
    ...
    def gradient_penalty(self, input_img, generate_img):  # Wgan-gp 梯度惩罚项
        # interpolate sample
